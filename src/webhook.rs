...
impl GfbioWebhook {
    ...
    async fn save_temp_file(&self, filename: &str, content: &[u8]) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn download_xml(&self, url: &str) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn send_gfbio_request(&self, input_file_url: &str, transformation_id: &str) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn fetch_result_data(&self, job_id: &str, result_file: &str) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    fn create_job_from_response(&self,
        ...
        let (job_id, status, result_file, finish_time) = if let Some(response) = gfbio_response {
            if let Some(job_data) = response.get("job") {
                ...
                let status = job_data.get("status")
                    .and_then(|s| s.as_str())
                    .unwrap_or("pending")
                    .to_string();

                let result_file = job_data.get("result_file")
                    .and_then(|s| s.as_str())
                ...
                ("unknown".to_string(), "processing".to_string(), "output/result.json".to_string(), None)
            }
        } else {
            ("unknown".to_string(), "processing".to_string(), "output/result.json".to_string(), None)
        };
    ...
    pub async fn handle_url_transformation(
        ...
        let filename = request.xml_url
            .split('/')
            .last()
            .unwrap_or("downloaded.xml")
            .to_string();

        let gfbio_response = match self.send_gfbio_request(&request.xml_url, &transformation_id).await {
        ...
        let job = self.create_job_from_response(gfbio_response, &request.xml_url, &transformation_id, &filename);

        let job_id = job.job_id.clone();
        let result_file = job.result_file.clone();

        self.fetch_result_data(&job_id, &result_file).await.map_err(|e| {
    ...
    pub async fn handle_transformation(
        ...
        let temp_file_url = self.save_temp_file(&filename, &content).await.map_err(|e| {
            (StatusCode::INTERNAL_SERVER_ERROR, Json(ErrorResponse {
        ...
        let gfbio_response = match self.send_gfbio_request(&temp_file_url, &transformation_id).await {
            Ok(response) => Some(response),
            Err(e) => {
                eprintln!("GFBio API error: {}", e);
                None
        ...
        let job = self.create_job_from_response(gfbio_response, &temp_file_url, &transformation_id, &filename);

        Ok(Json(JobResponse { job }))
    }

    pub async fn get_job_status(&self, job_id: &str) -> Result<Json<JobResponse>, (StatusCode, Json<ErrorResponse>)> {
